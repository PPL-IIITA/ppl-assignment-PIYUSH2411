...
public class Ques2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try{
            ...
            BufferedWriter bw1 = new BufferedWriter(new FileWriter("happy.csv"));
            BufferedWriter bw2 = new BufferedWriter(new FileWriter("compatible_couple.csv"));
            for(i = 0; i < cg/2; i++){
                if(G[i].status.equals("committed")){
                    String timeStamp = new SimpleDateFormat("dd.MM.yyyy").format(new Date());
                    String timeStamp1 = new SimpleDateFormat("hh.mm.ss").format(new Date());
                    str = G[i].name+" is happy compitable with "+G[i].bf+" on "+timeStamp+" at "+timeStamp1;
                    bw2.write(str+"\r\n");
                    bw2.flush();
                }
            }
            for(i = 0; i < cg; i++){
                if(G[i].status.equals("committed")){
                    ...
                    String str1 = G[i].name+" is happy with "+G[i].bf+" on "+timeStamp+" at "+timeStamp1;
                    bw.write(str+ "\r\n");
                    bw1.write(str1+ "\r\n");
                    bw.flush();
                    bw1.flush();
